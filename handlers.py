            ...
            if event.message.is_reply:
                ...
                if signal:
                    ...
                    if lower_text in {"close all", "close at entry"}:
                        ...
                        await event.respond(f"Signal updated: {lower_text}.")
                    ...
                    if lower_text.startswith("move sl to "):
                        try:
                            ...
                            update_signal_json_and_dropbox(signal)
                            await event.respond(f"Signal updated: move SL to {new_sl}.")
                            return  # EARLY RETURN here to bypass AI sanitization
                        except Exception:
                    ...
                    if re.fullmatch(r"\+\d+\s*pips", lower_text):
                        signal["manipulation"] = "cancel_pending"
                        update_signal_json_and_dropbox(signal)
                        await event.respond("Signal updated: all pending trades canceled due to profit message.")
                        return  # EARLY RETURN

                    # Unknown manipulation
                    await event.respond("Unrecognized update command.")
                    return  # EARLY RETURN
